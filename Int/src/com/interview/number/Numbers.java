...
public class Numbers {
	...
	/*
	 * No. 15 3sum Given an array S of n integers, are there elements a, b, c in
	 * S such that a + b + c = 0? Find all unique triplets in the array which
	 * gives the sum of zero. Note: Elements in a triplet (a,b,c) must be in
		...
		for (int i = 0; i < num.length - 2; i = increment(num, i)) {
			...
			while (lo < hi) {
				...
				if (a + b + c == 0) {
					...
					listSet.add(list); // if (!listSet.contains(list))
										// listSet.add(list);
	...
	/**
	 * No. 16 3sum closet Given an array S of n integers, find three integers in
	 * S such that the sum is closest to a given number, target. Return the sum
	...
	public int firstMissingPositive(int[] A) {
		...
		while (i < A.length) {
			// 3 cases to swap:
			// A[i] is within [1..N]
			// A[i] is in right position
			// A[i] and A[A[i] - 1] are different (equal will result in dead
			// loop)
		...
		// find the A[i] in wrong position
	...
	public void nextPermutation(int[] num) {
		...
		// find longest descending tail and reverse it, num[curr - 1] is the
		// partition number
		int curr = num.length - 1;
		while (curr > 0 && num[curr - 1] >= num[curr])
			curr--;
	...
	/**
	 * No. 128 Longest Consecutive Sequence Given an unsorted array of integers,
	 * find the length of the longest consecutive elements sequence. For
	 * example, Given [100, 4, 200, 1, 3, 2], The longest consecutive elements
	 * sequence is [1, 2, 3, 4]. Return its length: 4. Your algorithm should run
	 * in O(n) complexity.
	 * 
	 * Time ~ O(N), Space ~ O(N) 因为要 O(n) Time，所以不能够 sort。 方法：用一个 HashSet
	 * 存放所有元素。然后对每个元素 i，分别搜索其邻近的左 (i--) 右 (i++) 是否存在 HashSet 中，若找到相应元素则长度加
	 * 1，直至搜索不到得到的长度为包含 i 的最长连续序列长度。通过遍历每个元素可找到最长的连续序列长度。 注意：每次在 HashSet
	 * 中找到一个元素后，必须将该元素删除。如果没有这步，之后该元素还可能被再次搜索到，从而导致 O(N^2) Time。
	 */
	...
	/**
	 * No. 166 Fraction to Recurring Decimal Given two integers representing the
	 * numerator and denominator of a fraction, return the fraction in string
	 * format.
	 * 
	...
	public String fractionToDecimal(int numerator, int denominator) {
		...
		while (r > 0) {
			if (map.containsKey(r)) {
				...
				map.put(r, str.length()); // no need to -1 on str.length() since
											// r hasn't been added to str
				r *= 10;
				str.append(r / d);
				r = r % d;
			}
	...
	/**
	 * No. 168 Excel Sheet Column Title Given a positive integer, return its
	...
	/**
	 * No. 171. Excel Sheet Column Number Given a column title as appear in an
	 * Excel sheet, return its corresponding column number.
	 * 
	 * For example:
	 * 
	 * A -> 1 B -> 2 C -> 3 ... Z -> 26 AA -> 27 AB -> 28 title to number
	 */
	...
	public int sqrt(int x) {
		...
		while (lo < hi) {
			int mid = (lo + hi) / 2;
			int div = x / mid; // don't compare x with mid * mid that will
								// overflow
			if (div > mid)
				lo = mid + 1;
			else if (div < mid)
	...
	/**
	 * No. 172 Given an integer n, return the number of trailing zeroes in n!.
	 * 
	...
	/**
	 * No. 189 rotate array Rotate an array of n elements to the right by k
	 * steps.
	 * 
	...
	/**
	 * No. 204 count primes Time ~ O(NlogNlogN), Space ~ O(N) 用埃氏筛法（详见上面第二个链接的
	 * Wikipedia），基本想法是：从 2 到 sqrt(n)，对每一个数 i，在 2 到 n 的范围内将 i^2 + k * i (k = 0,
	 * 1, 2, ...) 筛去（标记为false，即为合数）；最后将所有标记为 true（即质数）的个数统计出来，即为所有质数的个数。
	 ...
	 * 
	 * For example, Given [3,2,1,5,6,4] and k = 2, return 5. 对于 large data 或
	 * data stream（N -> infinity）无法 maintain size 为 N 的 heap，可以用 size 为 K 的
	 * heap，仅当新元素大于 heap min 时再 insert。
	 */
	...
	/**
	 * another way 2. Max PQ: Time ~ O(N + KlogN), Space ~ O(N) 定义一个 Comparator
	 * 使 java.util.PriorityQueue 变为 MaxPQ，这样只要 delMax() K 次即可。
	 */
	...
	/**
	 * No. 220 Contains Duplicate I, II, III
	 */

	/**
	 * Contains Duplicate III Given an array of integers, find out whether there
	 * are two distinct indices i and j in the array such that the difference
	...
	public boolean containsNearbyAlmostDuplicate(int[] nums, int k, int t) {
		...
		for (int i = 0; i < nums.length; ++i) {
			 ...
			 * ?Integer.MIN_VALUE:nums[i]-t; int ceiling =
			 * nums[i]>Integer.MAX_VALUE-t ?Integer.MAX_VALUE:nums[i]+t;
			 */
	...
	/**
	 * No. 219 Contains Duplicate II Given an array of integers and an integer
	 * k, find out whether there there are two distinct indices i and j in the
	 * array such that nums[i] = nums[j] and the difference between i and j is
	 * at most k.
	 */
	...
	public boolean containsNearbyDuplicate(int[] nums, int k) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			if (!map.containsKey(val))
				map.put(val, i);
			else if (i - map.get(val) <= k)
				return true;
			else
				map.put(val, i); // update the map with the lated index if there
									// is replicated value, but doesn't qualify
	...
	/**
	 * No. 217 Contains Duplicate I Given an array of integers, find if the
	...
	/**
	 * No. 169 Given an array of size n, find the majority element. The majority
	...
	/**
	 * No. 229 Given an integer array of size n, find all elements that appear
	 * more than ⌊ n/3 ⌋ times. The algorithm should run in linear time and in
	 * O(1) space.
	 * 
	 * @param nums
	 * @return
	...
	/**
	 * No. 163 Find missing ranges Given a sorted integer array where the range
	 * of elements are [lower, upper] inclusive, return its missing ranges. For
	 * example, given [0, 1, 3, 50, 75], lower = 0 and upper = 99, return ["2",
	 * "4->49", "51->74", "76->99"].
	 */
	...
	/**
	 * No. 228 Given a sorted integer array without duplicates, return the
	...
	public int calculate2(String s) {
		...
		int num = 0;
		char sign = '+';  // a symbal operator as '+' to indicate 
		for (int i = 0; i < len; i++) {
			...
			if ((!Character.isDigit(s.charAt(i)) && ' ' != s.charAt(i))
				...
				// evaluate the the previous sign and update the stack accordingly. 
				if (sign == '-') {
					stack.push(-num);
				}
				if (sign == '+') {
				...
				sign = s.charAt(i); // just update the sign. 
				num = 0;
			}
		}
	...
	/**
	 * No. 224 Implement a basic calculator to evaluate a simple expression
	 * string.
	 * 
	 ...
	 * attention:
	 * 
	 * digit: it should be one digit from the current number '+': number is
	 * over, we can add the previous number and start a new number '-': same as
	 * above '(': push the previous result and the sign into the stack, set
	 * result to 0, just calculate the new result within the parenthesis. ')':
	 * pop out the top two numbers from stack, first one is the sign before this
	 * pair of parenthesis, second is the temporary result before this pair of
	 * parenthesis. We add them together. Finally if there is only one number,
	 * from the above solution, we haven't add the number to the result, so we
	 * do a check see if the number is zero.
	 */
	...
	/**
	 * No. 12 Question: Given an integer, convert it to a roman numeral. Input
	 * is guaranteed to be within the range from 1 to 3999. Hint: What is the
	 * range of the numbers? Solution: Difficulty: Medium, Frequency: Low ￼￼ ￼
	 * Roman Literal ￼ Decimal I 1 ￼ V 5 ￼ X 10 ￼ L 50 ￼ C 100 ￼ D ￼ 500 M 1000
	 */
	...
	/**
	 * No. 13 Given a roman numeral, convert it to an integer. Input is
	 * guaranteed to be within the range from 1 to 3999. Solution: Difficulty:
	 * Medium, Frequency: Low ￼￼ Roman Literal Decimal I 1 V 5 ￼ X 10 L 50 C ￼
	 * 100 D ￼ 500 ￼ ￼M 1000
	 */
	...
	public int romanToInt(String s) {
		...
		for (char c : s.toCharArray()) {
			int curr = map.get(c);
			total += (curr > prev) ? (curr - 2 * prev) : curr; // use subtractive rule
			prev = curr;
		}
		return total;
	}
	...
	/**
	 * No. 56 Given a collection of intervals, merge all overlapping intervals.
	...
	public List<Interval> merge(List<Interval> intervals) {
		...
		while (next < intervals.size()) {
			if (intervals.get(curr).end < intervals.get(next).start) {
				...
				intervals.get(curr).end = Math.max(intervals.get(curr).end,
	...
	/**
	 * No. 57 Given a set of non-overlapping intervals, insert a new interval
	...
	public List<Interval> insert(List<Interval> intervals, Interval newInterval) {
		...
		for (Interval i : intervals) {
			if (newInterval == null || i.end < newInterval.start) // this new
																	// Interval
																	// not
																	// affect
																	// the
																	...
																	// one
				result.add(i);
			else if (i.start > newInterval.end) { // this newInterval not affect
													// the existing
